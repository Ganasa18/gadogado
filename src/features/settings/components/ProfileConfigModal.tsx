import { useState, useEffect } from "react";
import { invoke } from "@tauri-apps/api/core";
import { motion } from "framer-motion";
import { Plus, Settings, Loader2, ChevronRight } from "lucide-react";
import type { DbConnection, DbAllowlistProfile } from "../../rag/types";
import { cn } from "../../../utils/cn";

interface ProfileConfigModalProps {
  connection: DbConnection | null;
  onClose: () => void;
}

export function ProfileConfigModal({ connection, onClose }: ProfileConfigModalProps) {
  const [profiles, setProfiles] = useState<DbAllowlistProfile[]>([]);
  const [selectedProfile, setSelectedProfile] = useState<DbAllowlistProfile | null>(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState<string | null>(null);

  useEffect(() => {
    if (connection) {
      loadProfiles();
    }
  }, [connection]);

  const loadProfiles = async () => {
    setLoading(true);
    setError(null);
    try {
      const result = await invoke<DbAllowlistProfile[]>("db_list_allowlist_profiles");
      setProfiles(result);
      if (result.length > 0) {
        setSelectedProfile(result[0]);
      }
    } catch (err) {
      const errorMsg = err instanceof Error ? err.message : String(err);
      setError(`Failed to load profiles: ${errorMsg}`);
    } finally {
      setLoading(false);
    }
  };

  const getProfileRules = () => {
    if (!selectedProfile) return null;
    try {
      return JSON.parse(selectedProfile.rules_json);
    } catch {
      return null;
    }
  };

  const rules = getProfileRules();

  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4"
      onClick={onClose}
    >
      <motion.div
        initial={{ scale: 0.95, opacity: 0, y: 20 }}
        animate={{ scale: 1, opacity: 1, y: 0 }}
        exit={{ scale: 0.95, opacity: 0, y: 20 }}
        className="bg-app-panel border border-app-border rounded-xl shadow-2xl w-full max-w-2xl overflow-hidden flex flex-col max-h-[80vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="px-6 py-4 border-b border-app-border flex justify-between items-center bg-app-card/30">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-app-accent/10 flex items-center justify-center text-app-accent">
              <Settings className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-lg font-semibold text-app-text">Profile Configuration</h2>
              <p className="text-xs text-app-subtext">{connection?.name || "Unknown Connection"}</p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 hover:bg-app-card rounded-full text-app-subtext transition-colors"
          >
            <Plus className="w-5 h-5 rotate-45" />
          </button>
        </div>

        {/* Content */}
        <div className="p-6 overflow-y-auto flex-1 space-y-6">
          {loading ? (
            <div className="flex items-center justify-center py-8">
              <Loader2 className="w-6 h-6 animate-spin text-app-accent" />
            </div>
          ) : error ? (
            <div className="bg-destructive/10 text-destructive border border-destructive/20 rounded-lg p-4 text-sm">
              {error}
            </div>
          ) : (
            <>
              {/* Profile Selector */}
              <div>
                <label className="block text-xs font-bold uppercase tracking-wider text-app-subtext mb-2">
                  Select Profile
                </label>
                <div className="relative">
                  <select
                    value={selectedProfile?.id || ""}
                    onChange={(e) => {
                      const profile = profiles.find(p => p.id === parseInt(e.target.value));
                      setSelectedProfile(profile || null);
                    }}
                    className="w-full appearance-none bg-app-card border border-app-border rounded-lg px-4 py-2.5 text-app-text text-sm focus:ring-2 focus:ring-app-accent/50 focus:border-app-accent outline-none transition-all cursor-pointer"
                  >
                    {profiles.map((profile) => (
                      <option key={profile.id} value={profile.id}>
                        {profile.name} - {profile.description || "No description"}
                      </option>
                    ))}
                  </select>
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-app-subtext">
                    <ChevronRight className="w-3.5 h-3.5 rotate-90" />
                  </div>
                </div>
              </div>

              {/* Profile Details */}
              {selectedProfile && rules && (
                <div className="space-y-4">
                  <h3 className="text-sm font-bold text-app-text uppercase tracking-wider">
                    Profile Settings
                  </h3>

                  <div className="bg-app-card/50 border border-app-border rounded-lg p-4 space-y-3">
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-app-subtext">Max Limit</span>
                      <span className="text-sm font-mono text-app-text">{rules.max_limit || 200}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-sm text-app-subtext">Allow Joins</span>
                      <span className={cn(
                        "text-xs font-bold uppercase tracking-wider px-2 py-1 rounded",
                        rules.allow_joins ? "bg-app-success/10 text-app-success" : "bg-app-subtext/10 text-app-subtext"
                      )}>
                        {rules.allow_joins ? "Enabled" : "Disabled"}
                      </span>
                    </div>
                    <div>
                      <span className="text-sm text-app-subtext block mb-2">Allowed Tables ({rules.allowed_tables ? Object.keys(rules.allowed_tables).length : 0})</span>
                      <div className="bg-app-bg border border-app-border rounded-lg p-3 max-h-32 overflow-y-auto">
                        {rules.allowed_tables && Object.keys(rules.allowed_tables).length > 0 ? (
                          <div className="flex flex-wrap gap-2">
                            {Object.keys(rules.allowed_tables).map((table) => (
                              <span
                                key={table}
                                className="text-xs px-2 py-1 bg-app-accent/10 text-app-accent rounded"
                              >
                                {table}
                              </span>
                            ))}
                          </div>
                        ) : (
                          <p className="text-xs text-app-subtext italic">No tables configured</p>
                        )}
                      </div>
                    </div>
                    <div>
                      <span className="text-sm text-app-subtext block mb-2">Denied Keywords</span>
                      <div className="flex flex-wrap gap-2">
                        {(rules.deny_keywords || []).map((keyword: string) => (
                          <span
                            key={keyword}
                            className="text-xs px-2 py-1 bg-destructive/10 text-destructive rounded"
                          >
                            {keyword}
                          </span>
                        ))}
                      </div>
                    </div>
                  </div>

                  <div className="bg-app-accent/10 border border-app-accent/20 rounded-lg p-4">
                    <p className="text-xs text-app-subtext leading-relaxed">
                      <strong className="text-app-accent">Note:</strong> Profile settings control which tables can be accessed and what queries are allowed. Use the "Manage Tables" option to configure accessible tables for this connection.
                    </p>
                  </div>
                </div>
              )}
            </>
          )}
        </div>

        {/* Footer */}
        <div className="p-6 bg-app-card/30 border-t border-app-border flex justify-end">
          <button
            onClick={onClose}
            className="px-5 py-2 text-sm font-medium text-app-subtext hover:text-app-text transition-all"
          >
            Close
          </button>
        </div>
      </motion.div>
    </motion.div>
  );
}
