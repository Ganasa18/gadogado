...
const loadSavedConfig = async (connId: number): Promise<{
  ...
  try {
    const config = await invoke<DbConnectionConfig>("db_get_connection_config", { connId });
    return {
...
// Save configuration to database (including selected columns)
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (savedConfig.profileId) {
        const savedProfile = profileList.find(p => p.id === savedConfig.profileId);
        if (savedProfile) {
  ...
  return (
    <div className="flex flex-col h-screen bg-app-bg text-app-text overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        {/* Main Content Area */}
        <main className="flex-1 flex flex-col bg-app-bg relative overflow-hidden">
              ...
              >
                ...
                <div className="grid grid-cols-12 gap-8">
                  <div className="col-span-8 space-y-6">
                    {/* Access Toggle Card */}
                    <div 
                      onClick={() => handleToggleTable(selectedTable.table_name)}
                      className={cn(
                    ...
                    <div className="bg-app-panel/40 border border-app-border rounded-2xl overflow-hidden shadow-sm">
                      ...
                      <div className="divide-y divide-app-border/40">
                        {loadingColumns.has(selectedTable?.table_name || "") ? (
                        ...
                        ) : columns.get(selectedTable?.table_name || "")?.length === 0 ? (
                          <div className="p-12 text-center space-y-3">
                          ...
                          columns.get(selectedTable?.table_name || "")?.map((col, idx) => (
                            <div
                              key={col.column_name}
                            ...
                            >
                              ...
                              <label className="flex items-center gap-2 cursor-pointer">
                                ...
                                <span className="text-xs text-app-subtext group-hover:text-app-text">
                                  {col.is_primary_key ? "Primary Key" : "Include in queries"}
                                </span>
                              </label>
                            </div>
                          ))
                    ...
                    <div className="bg-app-panel/40 border border-app-border rounded-2xl p-6 space-y-6">
                      ...
                      <div className="pt-4 border-t border-app-border">
                        <p className="text-[11px] text-app-subtext leading-relaxed">
                          Applied profile restricts queries to indexed columns only. Keywords like <span className="text-destructive/80 font-mono">password</span> are auto-redacted.
                        </p>
                      </div>
                    </div>
