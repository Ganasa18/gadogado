...
export default function AiResultsPage() {
  ...
  const modelOptions = useMemo(() => {
    ...
    if (provider === "gemini") {
      return ["gemini-2.5-flash-lite", "gemini-2.0-flash-lite", "gemini-3-flash-preview"];
    }
    if (provider === "openai") {
  ...
  const canGenerate = selectedModel !== "" && selectedModel !== "No models found";

  const summariesByCheckpoint = useMemo(() => {
    return summaries.reduce<Record<string, QaCheckpointSummary>>((acc, summary) => {
  ...
  const loadOutputs = async () => {
    ...
    setLoading(true);
    try {
      const [checkpointData, summaryData, testCaseData, runData] = await Promise.all([
        invoke<QaCheckpoint[]>("qa_list_checkpoints", { sessionId }),
        invoke<QaCheckpointSummary[]>("qa_list_checkpoint_summaries", { sessionId }),
  ...
  const activeSummary = activeCheckpoint ? summariesByCheckpoint[activeCheckpoint.id] : null;
  const activeTestCases = activeCheckpoint ? testCasesByCheckpoint[activeCheckpoint.id] || [] : [];
      ...
      <div className="flex-none bg-app-bg border-b border-app-border z-10 shadow-sm">
        ...
        <div className="px-5 pt-6 pb-6 grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
             <div className="md:col-span-3 flex items-center gap-2 text-xs text-app-text">
                ...
                <span className="font-semibold">{checkpoints.length} Checkpoints</span>
             </div>
             ...
             <div className="md:col-span-9 flex flex-wrap items-center justify-end gap-3">
                 ...
                 <div className="relative min-w-[180px]">
                    ...
                    >
                      {modelOptions.map((opt) => (
                        <option key={opt} value={opt} className="bg-app-panel text-app-text">{opt}</option>
                      ))}
                    </select>
                 ...
                 <div className="relative min-w-[120px]">
                    ...
                    >
                      {LANGUAGES.map((lang) => (
                        <option key={lang} value={lang} className="bg-app-panel text-app-text">{lang}</option>
                      ))}
                    </select>
                     <ChevronDown className="absolute right-2 top-2 w-3.5 h-3.5 text-app-subtext pointer-events-none" />
                 </div>
           ...
           {checkpoints.length === 0 ? (
             ...
             <div className="flex flex-col">
                {checkpoints.map((checkpoint) => {
                   ...
                   const testCount = (testCasesByCheckpoint[checkpoint.id] || []).length;
                     ...
                     >
                        ...
                        <div className="flex justify-between items-start mb-1">
                           <div className="flex items-center gap-2 font-mono text-[10px] text-app-subtext opacity-70">
                              ...
                              <span>{new Date(checkpoint.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}</span>
                           </div>
                           <div className="flex gap-1">
                              {hasSummary && <div className="w-1.5 h-1.5 rounded-full bg-emerald-500 shadow-sm shadow-emerald-500/50" />}
                              {testCount > 0 && <div className="w-1.5 h-1.5 rounded-full bg-sky-500 shadow-sm shadow-sky-500/50" />}
                           </div>
                        </div>
                        <div className={`text-sm font-medium line-clamp-2 mb-2 ${isActive ? "text-app-text" : "text-app-subtext group-hover:text-app-text"}`}>
           ...
           {!activeCheckpoint ? (
             ...
             <div className="p-6 max-w-5xl mx-auto space-y-8 animate-in fade-in duration-300 transform-gpu key={activeCheckpoint.id}">
                ...
                <div className="flex items-start justify-between gap-4 border-b border-app-border/40 pb-4">
                   <div>
                      <div className="flex items-center gap-3 mb-1">
                         <span className="font-mono text-2xl font-light text-emerald-400 opacity-80">#{activeCheckpoint.seq}</span>
                         <h2 className="text-xl font-semibold text-app-text">{activeCheckpoint.title || "Untitled Checkpoint"}</h2>
                      </div>
                      <div className="flex items-center gap-3 text-xs text-app-subtext">
                         <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5" /> {new Date(activeCheckpoint.createdAt).toLocaleString()}</span>
                         <span className="w-1 h-1 rounded-full bg-app-border" />
                         <span className="flex items-center gap-1.5"><Fingerprint className="w-3.5 h-3.5" /> Events {activeCheckpoint.startEventSeq} - {activeCheckpoint.endEventSeq}</span>
                      </div>
                   </div>
                   ...
                   <div className="flex items-center gap-2">
                       {!activeSummary && (
                          ...
                          >
                            ...
                            {actionCheckpoint === activeCheckpoint.id ? "Analyzing..." : "Generate Summary"}
                          </button>
                       )}
                       <button
                        ...
                        >
                          ...
                          {actionCheckpoint === activeCheckpoint.id ? "Generating..." : "Generate Tests"}
                        </button>
                   </div>
                </div>
                ...
                {activeSummary && (
                   <section className="space-y-3">
                      ...
                      <div className="bg-app-card/60 rounded-xl border border-app-border p-5 shadow-sm transition-all hover:border-app-border/80">
                         ...
                         {(activeSummary.entitiesJson || activeSummary.risksJson) && (
                            <div className="flex flex-wrap gap-2 mt-4 pt-4 border-t border-app-border/30">
                              <SummaryMeta label="Entities" jsonValue={activeSummary.entitiesJson} icon={<Layers className="w-3 h-3" />} />
                              <SummaryMeta label="Risks" jsonValue={activeSummary.risksJson} icon={<AlertTriangle className="w-3 h-3 text-amber-400" />} />
                            </div>
                         )}
                      </div>
                   </section>
                ...
                <section className="space-y-4">
                   <div className="flex items-center justify-between">
                      <h3 className="text-sm font-semibold uppercase tracking-wider text-app-subtext flex items-center gap-2">
                         <CheckCircle2 className="w-4 h-4 text-sky-400" /> Generated Tests
                         <span className="ml-1 text-app-subtext/60 bg-white/5 px-2 py-0.5 rounded-full text-xs">{activeTestCases.length}</span>
                      </h3>
                   </div>
                   ...
                   {activeTestCases.length > 0 ? (
                         ...
                         {groupedCases.positive.length > 0 && (
                            <div className="space-y-3">
                               <div className="flex items-center gap-2 pb-1 border-b border-app-border/30">
                                  ...
                                  <h4 className="text-xs font-bold uppercase tracking-wider text-emerald-400 opacity-90">Positive Scenarios</h4>
                                  <span className="text-[10px] text-app-subtext">({groupedCases.positive.length})</span>
                               </div>
                               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  {groupedCases.positive.map(tc => <TestCaseCard key={tc.id} testCase={tc} />)}
                               </div>
                            </div>
                         )}
                         ...
                         {groupedCases.negative.length > 0 && (
                            <div className="space-y-3">
                               <div className="flex items-center gap-2 pb-1 border-b border-app-border/30">
                                  ...
                                  <h4 className="text-xs font-bold uppercase tracking-wider text-rose-400 opacity-90">Negative Scenarios</h4>
                                  <span className="text-[10px] text-app-subtext">({groupedCases.negative.length})</span>
                               </div>
                               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  {groupedCases.negative.map(tc => <TestCaseCard key={tc.id} testCase={tc} />)}
                               </div>
                            </div>
                         )}
                         ...
                         {groupedCases.edge.length > 0 && (
                            <div className="space-y-3">
                               <div className="flex items-center gap-2 pb-1 border-b border-app-border/30">
                                  ...
                                  <h4 className="text-xs font-bold uppercase tracking-wider text-amber-400 opacity-90">Edge Cases</h4>
                                  <span className="text-[10px] text-app-subtext">({groupedCases.edge.length})</span>
                               </div>
                               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  {groupedCases.edge.map(tc => <TestCaseCard key={tc.id} testCase={tc} />)}
                               </div>
                            </div>
                         )}
                         ...
                         {groupedCases.exploratory.length > 0 && (
                            <div className="space-y-3">
                               <div className="flex items-center gap-2 pb-1 border-b border-app-border/30">
                                  ...
                                  <h4 className="text-xs font-bold uppercase tracking-wider text-violet-400 opacity-90">Exploratory / Other</h4>
                                  <span className="text-[10px] text-app-subtext">({groupedCases.exploratory.length})</span>
                               </div>
                               <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                                  {groupedCases.exploratory.map(tc => <TestCaseCard key={tc.id} testCase={tc} />)}
                               </div>
                            </div>
                         )}
                      </div>
                      ...
                      <div className="flex flex-col items-center justify-center py-10 border-2 border-dashed border-app-border/40 rounded-xl bg-app-panel/20 text-center">
                         ...
                         <p className="text-sm text-app-subtext">No test cases generated yet.</p>
                         <p className="text-xs text-app-subtext/50 mt-1">Use "Generate Tests" to create scenarios.</p>
                      </div>
                   )}
                </section>
             </div>
...
function SummaryMeta({ label, jsonValue, icon }: { label: string; jsonValue?: string | null, icon?: React.ReactNode }) {
...
function TestCaseCard({ testCase }: { testCase: QaTestCase }) {
  ...
  const typeConfig = useMemo(() => {
    switch(testCase.type) {
      case "positive": return { color: "emerald", icon: CheckCircle2 };
      case "negative": return { color: "rose", icon: ShieldAlert };
      case "edge": return { color: "amber", icon: AlertTriangle };
      case "exploratory": return { color: "violet", icon: Search };
      default: return { color: "slate", icon: BookOpen };
    }
  }, [testCase.type]);
  ...
  let badgeClass = "";
  if (testCase.type === "positive") badgeClass = "bg-emerald-500/10 text-emerald-400 border-emerald-500/20";
  else if (testCase.type === "negative") badgeClass = "bg-rose-500/10 text-rose-400 border-rose-500/20";
  else if (testCase.type === "edge") badgeClass = "bg-amber-500/10 text-amber-400 border-amber-500/20";
  else if (testCase.type === "exploratory") badgeClass = "bg-violet-500/10 text-violet-400 border-violet-500/20";
  else badgeClass = "bg-slate-500/10 text-slate-400 border-slate-500/20";

  return (
    <div className={`rounded-xl border ${expanded ? 'border-app-border shadow-md bg-black/40' : 'border-app-border/40 bg-black/20'} hover:border-app-border/80 transition-all duration-200`}>
       <div 
       ...
       >
         ...
         <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between gap-2">
               <h6 className="text-sm font-medium text-app-text truncate pr-2">{testCase.title}</h6>
               <div className="flex items-center gap-2">
                  {testCase.priority && (
                    <span className="text-[10px] uppercase font-bold tracking-wider text-app-subtext/70 bg-white/5 px-1.5 py-0.5 rounded">{testCase.priority}</span>
                  )}
                  {expanded ? <ChevronDown className="w-4 h-4 text-app-subtext" /> : <ChevronRight className="w-4 h-4 text-app-subtext" />}
               </div>
            </div>
            {!expanded && (
               <div className="text-xs text-app-subtext mt-1 flex gap-3 items-center">
                  ...
                  {testCase.expected && (
                     <>
                        ...
                        <span className="opacity-50 truncate max-w-[400px]">Expects: {testCase.expected}</span>
                     </>
                  )}
               </div>
            )}
            ...
            <div className="space-y-4 pt-2 border-t border-app-border/30">
               ...
               {testCase.expected && (
                 <div className="bg-emerald-500/5 rounded-lg p-3 border border-emerald-500/10 flex gap-3 items-start">
                    ...
                    <div>
                      <div className="text-[10px] uppercase tracking-wider text-emerald-500/70 mb-1 font-semibold">Expected Result</div>
                      <div className="text-sm text-app-text/90">{testCase.expected}</div>
                    </div>
                 </div>
               )}
            </div>
...
function TestSteps({ stepsJson }: { stepsJson: string }) {
  ...
  if (steps.length === 0) return <div className="text-xs text-app-subtext italic">No steps defined.</div>;

  return (
    <div className="space-y-2">
      {steps.map((step, i) => (
        <div key={i} className="flex gap-3 text-xs text-app-subtext group hover:text-app-text transition-colors">
           <span className="font-mono opacity-40 select-none w-5 text-right flex-none tabular-nums text-[11px] pt-px">{i + 1}.</span>
