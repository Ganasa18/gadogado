...
export default function SessionDetailPage() {
  ...
  const { screenshots: allScreenshots, loading: screenshotsLoading, reload: reloadScreenshots } = useQaScreenshots(sessionId);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start max-w-full">
          ...
          <div className="lg:col-span-8 flex flex-col gap-6 order-2 lg:order-1">
             <section className="space-y-6">
                <RecordingSummaryCard session={session} isRecording={isRecording} />
                      ...
                      <div className="p-4 space-y-5">
                          ...
                          <div className="space-y-2">
                             <div className="flex items-center justify-between text-[11px] text-app-subtext">
                                <span className="flex items-center gap-1.5"><Cpu className="w-3 h-3" /> Model Provider</span>
        ...
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-3xl bg-app-panel border border-app-border rounded-2xl shadow-2xl p-5 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between gap-3 mb-4">
              <div>
                <div className="text-xs uppercase tracking-widest text-app-subtext">API Replay</div>
                <div className="text-sm font-semibold text-app-text">cURL Command Preview</div>
              </div>
              <button
                type="button"
            ...
            <div className="mt-4 flex items-center justify-between">
              <span className="text-[10px] text-app-subtext">
                {curlCopied ? <span className="text-emerald-400">Copied to clipboard!</span> : "Use this in your terminal."}
              </span>
              <button
                type="button"
      ...
      {apiReplayModalOpen && (
        <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4">
          <div className="w-full max-w-4xl bg-app-panel border border-app-border rounded-2xl shadow-2xl p-5 animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between gap-3 mb-4">
              <div>
                <div className="text-xs uppercase tracking-widest text-app-subtext">API Replay Queue</div>
                <div className="text-sm font-semibold text-app-text">Request + Response Timeline</div>
              </div>
              <button
                type="button"
              ...
              {apiReplayItems.map((item, index) => (
                <div
                  ...
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <span className="text-[10px] uppercase tracking-wide opacity-60">Step {index + 1}</span>
                    <span className={`text-[10px] uppercase font-bold tracking-wider ${
                        item.status === 'success' ? 'text-emerald-400' : 
                        item.status === 'error' ? 'text-red-400' :
                        item.status === 'running' ? 'text-sky-400' : 'text-app-subtext'
                    }`}>{item.status}</span>
                  </div>
                  <div className="font-mono text-app-text opacity-90 truncate">{item.requestLabel}</div>
                  {item.responseLabel && (
