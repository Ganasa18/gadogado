...
pub struct DbQueryTelemetry {
    ...
    /// Enriched/rewritten query from intent enricher (for debugging)
    #[serde(skip_serializing_if = "Option::is_none")]
    pub enriched_query: Option<String>,
    /// Detected intent from enricher (for debugging)
    #[serde(skip_serializing_if = "Option::is_none")]
...
// ============================================================
// QUERY INTENT ENRICHMENT TYPES
// ============================================================

/// Classified query intent from the enricher
#[derive(Debug, Clone, Serialize, Deserialize, Default)]
...
/// Result from the Query Intent Enricher
#[derive(Debug, Clone, Serialize, Deserialize)]
...
impl EnrichedQuery {
    /// Create a passthrough (no enrichment) from the raw query
    pub fn passthrough(query: &str) -> Self {
        Self {
            original_query: query.to_string(),
            rewritten_query: query.to_string(),
            detected_intent: QueryIntent::SimpleSelect,
            detected_tables: vec![],
            detected_operation: "unknown".to_string(),
            confidence: 0.0,
        }
    }

    /// Whether enrichment actually changed the query
    pub fn was_enriched(&self) -> bool {
        self.confidence > 0.0 && self.original_query != self.rewritten_query
    }
}
...
pub struct LlmTemplateSelection {
    ...
    /// JOIN-related fields (for LEFT JOIN templates)
    #[serde(default)]
    pub related_table: Option<String>,
    #[serde(default)]
    pub foreign_key_column: Option<String>,
    #[serde(default)]
    pub main_table_columns: Option<String>,
    #[serde(default)]
    pub related_table_columns: Option<String>,
    pub confidence: f32,
